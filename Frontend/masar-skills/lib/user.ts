...
export default async function createUser({firstName, lastName, email, password, role, phoneNumber}: types) {
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 409 || result.message?.includes('email already exists')) {
        return {
          errors: {
    ...
    if (error instanceof TypeError && error.message.includes('fetch')) {
      return {
        errors: {
          email: "Network error: Unable to connect to the server. Please check if the server is running."
        }
      };
    }
...
export async function getUserById(id: number) {
  try {
    const response = await fetch(
      `http://localhost:5236/api/Users/${id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export async function getCourseEnrollment(token:string){
  try {
    const response = await fetch(
...
export async function getQuizzes(){
  try {
    const response = await fetch(
      `http://localhost:5236/api/Quiz/start/1`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
...
export async function getCourseByID(id:number){
  try {
    const response = await fetch(
      `http://localhost:5236/api/courses/${id}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
